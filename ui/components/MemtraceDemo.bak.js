...
import TimeFlameGraphComponent from "../libs/tpt-ui-lib/components/time-flame-graph/TimeFlameGraph"
import LineChart from "../libs/tpt-ui-lib/components/line-chart/LineChart"
...
export default class MemtraceDemo extends React.Component {
    ...
    _onUrlBtnClicked() { // download file
        const url = this._inputRef.current.value;
        if (!url) {
            alert("URL不能为空!");
            return;
        }

        RPC.invoke("download_file", { url: url }).then(result => {
            if (result && result.ok) {
    ...
    _onMessage(event) {
        ...
        if (msg.type == "open_report" && msg.data.report_path) {
            let files = [msg.data.report_path];
    ...
    render() {
        ...
        return (
            <div ref={this._ref} style={{ height: "100%", display: "flex", "flexDirection": "column" }}>
                ...
                <Modal
                    size="sm"
                    show={this.state.showRemoteModal}
                    onHide={() => this.setState({ showRemoteModal: false })}
                    ...
                    <Modal.Body>
                        <Form>
                            <Form.Group as={Row} controlId="formHorizontalEmail">
                                <Col sm={12}>
                                    <Form.Control ref={this._inputRef} id="urlInput" type="text" placeholder="http://" />
                                </Col>
                            </Form.Group>
                            <Form.Group as={Row}>
                                <Button id="remoteIPConnectBtn" block style={{ margin: "0 10px" }} onClick={(e) => this._onUrlBtnClicked()}>Open</Button>
                            </Form.Group>
                        </Form>
                    </Modal.Body>
                </Modal>
